...
Lando offers a [configurable recipe](./../recipes/symfony40.md) for spinning up [Symfony 4.0](https://symfony.com) apps. Let's go over some basic usage.
...
*Note*: Be aware that Lando will automatically point to your `/public` folder. So you should only change `webroot` if your
...
Once you've completed the above you should be able to start your Symfony 4.0 site.
...
If you visit any of the green-listed URLs that show up afterwards you should be welcomed with the Symfony 4.0 welcome screen.
...
Each Lando Symfony 4.0 recipe will also ship with helpful dev utilities. This means you can use things like `console`, `phpunit`, `composer` and `php-cli` via Lando and avoid mucking up your actual computer trying to manage `php` versions and tooling.
...
lando phpunit                  Executes PHP Unit tests located in the app/tests folder.
```
...
You can take advantage of Lando's [events framework](./../config/events.md) to automate common tasks. Here are some useful examples you can drop in your `.lando.yml` to make your Symfony 4.0 app super slick.
