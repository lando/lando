...
**BUT IF YOU"RE AWESOME AND WANT TO HELP US, USE IT AND BREAK IT AND SUBMIT ISSUES.** If you do experience any issues try the following corrective action first:
  ...
  1. `lando destroy` and then `lando start` any failing apps
  2. Restart the docker daemon
  3. Restart your computer
  4. `docker rm -f CONTAINER_ID` any containers throwing errors
  5. Reset the docker daemon to factory defaults (this will blow up all your containers)
  6. Revert to [beta.35](https://github.com/lando/lando/releases/tag/v3.0.0-beta.35) :(
