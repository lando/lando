...
You can easily boot up a best practices stack to run Drupal 7  by adding the following to your app's `.lando.yml`.
...
You can easily configure/override the Drupal 7 recipe and add in additional `.lando.yml` config such as `services`, `tooling`, `proxy setting` and `sharing`.
