 ...
 *
 * id: sql-export-example
 * runs-from: ../examples/sql-export
 */
...
describe('sql-export-example', function() {
  this.retries(6);
  ...
  it('boot up a sql export example', done => {
    process.chdir(path.resolve(__dirname, '../examples/sql-export'));
  ...
  it('export the contents of the dbs', done => {
    process.chdir(path.resolve(__dirname, '../examples/sql-export'));
    const cli = new CliTest();
    cli.exec('lando db-export && lando db-export -h database2').then(res => {
      if (res.error === null) {
        done();
      } else {
        done(res.error);
