 ...
 *
 * title: proxy-format-example
 * src: examples/trivial-proxy
 */
...
describe('trivial-proxy', () => {
  ...
  it('test 1', done => {
    process.chdir('examples/trivial-proxy');
    const cli = new CliTest();
    cli.exec('node ../../bin/lando.js ssh web -c "true"').then(res => {
      if (res.error === null) {
        done();
      } else {
        done(res.error);
