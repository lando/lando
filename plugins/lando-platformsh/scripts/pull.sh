...
if [ "$DATABASE" != "none" ]; then
  ...
  TABLES=$(mysql --user=platformsh --password=platformsh --database=platformsh --host=database --port=3306 -e 'SHOW TABLES' | awk '{ print $1}' | grep -v '^Tables' )
  echo "Destroying all current tables in database... "
  for t in $TABLES; do
    echo "Dropping $t table from lando database..."
    mysql --user=platformsh --password=platformsh --database=platformsh --host=database --port=3306 -e "DROP TABLE $t"
  done
  ...
  if [ "$FRAMEWORK" != "wordpress" ]; then
    ...
    if drush sa | grep @platformsh.$SITE.$DATABASE 2>&1; then
      ...
      if [ "$DATABASE" != "live" ]; then
        ...
        if [ "$FRAMEWORK" == "drupal8" ]; then
          drush @platformsh.$SITE.$DATABASE cr all --strict=0
        else
          drush @platformsh.$SITE.$DATABASE cc all --strict=0
        fi
      fi
