...
var inspectDir = function(app) {
  ...
  // Get the presumed location of the lando.yaml
  var filepath = path.join(app.dir, config.appConfigFilename);

  // If that file does not exist then queue up for cleanup and dont try to load the landoyml
  if (!fs.existsSync(filepath)) {
    log.info('App %s appears to have been removed.', app.name);
    log.info('Queuing up %s for cleanup.', app.name);
    cacheBadApp(app);
    return null;
  }
  ...
  // Handle no entry error.
  .catch(function(err) {
    log.error('Failed to load config: %s %j', filepath, err);
  })
  ...
  .then(function() {
    ...
    // Get the landoyml
    var json = yaml.load(filepath);

    // And do the right sorting
    if (!json) {
      return null;
    }
    else if (!json.name) {
