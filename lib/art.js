...
exports.appDestroy = ({name, phase = 'pre'} = {}) => {
  switch (phase) {
    ...
    case 'pre':
      return chalk.cyan(`Preparing to consign ${appName(name)} to the dustbin of history...`);
    case 'post':
      return [
...
exports.appRebuild = ({name, phase = 'pre'} = {}) => {
  switch (phase) {
    ...
    case 'recovered':
      return exports.appStart({phase: 'recovered'});
  }
};
...
exports.appRestart = ({name, phase = 'pre'} = {}) => {
  switch (phase) {
    case 'error':
      return exports.appStart({phase: 'error'});
    case 'pre':
  ...
  switch (phase) {
    case 'error':
      return [
        ...
        '',
        'Looks like something went wrong starting your app!',
        'Lando is now attempting to repair your app automatically.',
        '',
        chalk.magenta(niceFont('Please stand by...', 'Small Slant')),
        '',
      ].join(os.EOL);
    case 'pre':
    ...
    case 'recovered':
      return [
        '',
        chalk.magenta('It appears Lando just recovered from a fatal error!'),
        '',
        'If you experience additional problems please consult the troubleshooting guide below.',
        chalk.green('https://docs.lando.dev/help/updating.html'),
        '',
      ].join(os.EOL);
  }
};
