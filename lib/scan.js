...
const request = axios.create({httpsAgent: new https.Agent({rejectUnauthorized: false})});
...
module.exports = (log = new Log()) => {
  ...
  const scanUrls = (urls, {max = 7, waitCodes = [400, 502, 404]} = {}) => {
    ...
    return Promise.map(urls, url => Promise.retry(() => {
      ...
      if (_.includes(url, '*')) return setOK(url);
      // Send REST request.
      return request.get(url)
      // Return good responses
      .then(response => setGood(url))
      // Retry waitcodes
      .catch(error => ((_.includes(waitCodes, error.response.status)) ? Promise.reject(setBad(url)) : setGood(url)));
    }, {max})
