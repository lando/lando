...
module.exports = class App {
  ...
  destroy() {
    ...
    .then(() => this.log.info(`Destroyed ${this.name}.`));
  };
  ...
  init() {
    ...
    // Log some things
    this.log.verbose(`Initiatilizing app ${this.name} from ${this.root}`);
  ...
  rebuild() {
    ...
    .then(() => this.log.info(`Rebuilt ${this.name}.`));
  };
 ...
 /**
  ...
  restart() {
    // Start it off
    this.log.info(`Restarting ${this.name}`);
    // stop/start
    return this.stop()
    .then(() => this.start())
    .then(() => this.log.info(`Restarted ${this.name}.`));
  };
  ...
  start() {
    ...
    .then(() => this.log.info(`Started ${this.name}.`));
  };
  ...
  stop() {
    ...
    .then(() => this.log.info(`Stopped ${this.name}.`));
  };
  ...
  uninstall(purge = false) {
    ...
    .then(() => this.log.info(`Uninstalled ${this.name}...`));
  };
};
