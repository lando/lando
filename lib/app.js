...
/*
 * Helper to init and then reportf
 */
...
module.exports = class App {
  ...
  destroy() {
    // Start it off
    this.log.info('destroying %s...', this.name);
    ...
    .then(() => this.log.info('destroyed %s.', this.name));
  };
  ...
  init() {
    ...
    // Log some things
    this.log.verbose('initiatilizing app from %s', this.root);
  ...
  rebuild() {
    // Start it off
    this.log.info('rebuilding %s...', this.name);
    ...
    .then(() => this.log.info('rebuilt %s.', this.name));
  };
 ...
 /**
  ...
  restart() {
    // Start it off
    this.log.info('restarting %s...', this.name);
    // stop/start
    return this.stop()
    .then(() => this.start())
    .then(() => this.log.info('restarted %s.', this.name));
  };
  ...
  start() {
    // Log
    this.log.info('starting %s...', this.name);
    ...
    .then(() => this.log.info('started %s.', this.name));
  };
  ...
  stop() {
    // Stop it!
    this.log.info('stopping %s...', this.name);
    ...
    .then(() => this.log.info('stopped %s.', this.name));
  };
  ...
  uninstall(purge = false) {
    // Cleaning up
    this.log.info('uninstalling %s...', this.name);
    ...
    .then(() => this.log.info('uninstalled %s.', this.name));
  };
};
