...
class App {
  ...
  start() {
    ...
    return this.inspect().then(() => this.metrics.report('start', utils.metricsParse(this)))
    ...
    // Scan Urls
    // @TODO: some config mechanism to strip out values?
    .then(() => this.scanner(_.flatMap(this.info, 'urls'), {max: 16}))
