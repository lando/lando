 ...
 * Instantiate
 */
var instantiate = function(name, config) {
  ...
  .then(function() {
    ...
    app.config = config || {};
    // Config home bind.
    app.config.homeBind = daemon.path2Bind4U(app.config.home);
  ...
  // Register app.
  .tap(function(app) {
    return registry.register({name: app.name, dir: app.config.appRoot});
  })
...
exports.list = function(opts) {
  ...
  .then(function(registeredApps) {
    return Promise.map(registeredApps, function(registeredApp) {

      // Get appConfig
      var configFile = path.join(registeredApp.dir, config.configFilename);
      var appConfig = yaml.safeLoad(fs.readFileSync(configFile));

      // Instantiate an app
      return instantiate(registeredApp.name, appConfig);
...
exports.cleanup = function(app) {
  ...
  return this.list({useCache: false})
