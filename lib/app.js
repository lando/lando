 ...
 */
const loadPlugins = app => Promise.resolve(app.plugins.registry)
  // Filter out
  .filter(plugin => _.has(plugin, 'app'))
  // LOADEM!
  .map(plugin => app.plugins.load(plugin, app, plugin.app))
...
class App {
  ...
  init() {
    ...
    // Get our base level ComposeService from the factory
    const ComposeService = this.factory.get('compose');
    // Combine some stuffs
    const globalOptions = {env: this.env, labels: this.labels};
    ...
    .then(() => {
      ...
      this.add(new ComposeService('globals-0', {services: utils.toObject(this.services)}, globalOptions));
