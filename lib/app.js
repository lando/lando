...
module.exports = class App {
  ...
  init() {
    ...
    .then(() => {
      // Get our appMounts
      const appMounts = _(this.services)
        // Objectify
        .map(service => _.merge({name: service}, _.get(this, `config.services.${service}`, {})))
        // Set the default
        ...
        // Combine together
        .map(config => ([config.name, {volumes: [`${this.root}:/app:${config.app_mount}`]}]))
        .fromPairs()
        .value();

      // Front load them
      this.add(new this.ComposeService('mounts', {}, {services: appMounts}), true);
