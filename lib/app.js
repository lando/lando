...
class App {
   ...
   */
  init() {
    // Get some builder things
    const ComposeService = this.factory.get('_compose');
    ...
    this.composeData = [new ComposeService('compose', ...composeFiles)];
    ...
    .then(() => {
      ...
      const globals = utils.toObject(this.services, {environment: this.env, labels: this.labels});
      this.add(new ComposeService('globals', {services: globals}), true);
