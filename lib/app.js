...
class App {
  constructor(
    ...
    // Other properties ?
    // @TODO: better place to put the below?
    this.labels['io.lando.src'] = configFile;
    // this.info = {};
    // this.labels = {};
    // this.mount = '/app';
    ...
    */
    ...
    // @TODO: label with lando yml directory so we can destroy orphaned containers?
  };
  ...
  info() {
     ...
     * });
     */
    return this.events.emit('pre-info')
    // Merge in defaults
    .then(() => {
      // Merge in defualt services
      this.info = _.merge(this.info, utils.getInfoDefaults(this));
      ...
      .each(container => this.engine.scan(container).then(data => {
        const info = this.info[container.service];
  ...
  init() {
    ...
    .tap(() => {
      ...
      _.forEach(this.services, service => {
        prime[service] = {};
        prime[service].environment = this.env;
        prime[service].labels = this.labels;
      });
