...
const ComposeService = require('./service');
 ...
 */
const dumpComposeData = (data, dir) => _(data)
  .map(data => _.merge({}, data, {file: path.join(dir, `${data.id}.yml`)}))
  .map(data => yaml.dump(data.file, data.data))
  .value();

/*
 * Helper to load raw docker compose files
 */
const getComposeFiles = (files, dir, version = '3.2') => _(utils.validateFiles(files, dir))
  .map((file, index) => ({id: `compose-${index}`, data: yaml.load(file)}))
  .map(data => _.merge({}, data, {data: {version}}))
  .value();

/*
 * Helper to get global data
 */
const getGlobals = (services, data = {}) => new ComposeService(
  utils.toObject(services),
  {},
  {},
  '3.2',
  data
...
/*
 * Helper to find app plugins
 */
const findPlugins = app => _(app.plugins.find([app.root]))
 ...
 */
const loadPlugins = app => Promise.resolve(_.concat(app.plugins.registry, findPlugins(app)))
  // Filter out
  ...
  // LOADEM!
  .map(plugin => app.plugins.load(plugin, app))
...
class App {
   ...
   */
  add(id, data, front = false) {
    if (front) this.composeData.unshift({id, data});
    else this.composeData.push({id, data});
  }
  ...
  init() {
    ...
    this.scanner = require('./scan')(this.log);
    this.services = [];
    this.urls = [];
    this.version = this.config.version || '3.2';
    // Get compose data if we have any, otherwise set to []
    this.composeData = getComposeFiles(_.get(this, 'config.compose', []), this.root, this.version);
    // Log some things
     ...
     */
    return loadPlugins(this).then(() => this.events.emit('pre-instantiate-app', this))
    ...
    .then(() => {
      ...
      // Front load our global shiz
      this.add('globals-0', getGlobals(this.services, {env: this.env, labels: this.labels}), true);
