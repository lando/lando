...
module.exports = class Shell {
  ...
  sh(cmd, {mode = 'exec', detached = false, cwd = process.cwd(), cstdio = 'inherit', silent = false} = {}) {
    ...
    const firstCmd = path.basename(_.first(cmd));
    this.log.debug('process %s running %s %s', id, firstCmd, _.last(cmd), {cstdio, silent, mode, detached});
    this.log.silly('process %s full command, running', id, cmd);
