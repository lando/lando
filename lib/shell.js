...
const spawn = (cmd, opts) => new Promise((resolve, reject) => {
  ...
  // We lose this!
  const entrypoint = _.first(cmd);
  const command = _.tail(cmd);

  // Log
  log.verbose('Running spawn %s with args %j', entrypoint, command);
  log.debug('In mode %s with detached %s', options.mode, options.detached);
  log.debug('With env', process.env);

  // Run the spawn
  const run = _spawn(entrypoint, command, options);
