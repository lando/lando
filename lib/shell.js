...
/*
 * Helper to parse a command into useful metadata
 */
 ...
 */
const bufferMe = (buffer, silent = false) => {
  if (!silent) console.log(_.trimEnd(buffer.toString('utf8'), os.EOL));
 ...
 */
const spawn = (run, {stdio}, silent = false, stdout = '', stderr = '') => {
  // Run the spawn
  return new Promise(resolve => {
    if (stdio === 'pipe' || stdio[1] === 'pipe') {
      run.stdout.on('data', buffer => {
        stdout = stdout + bufferMe(buffer, silent);
      });
    }
    if (stdio === 'pipe' || stdio[2] === 'pipe') {
      run.stderr.on('data', buffer => {
        stderr = stderr + bufferMe(buffer, silent);
      });
    }
    run.on('error', buffer => {
      stderr = stderr + bufferMe(buffer, silent);
...
module.exports = class Shell {
  ...
  sh(cmd, {mode = 'exec', detached = false, cwd = process.cwd(), cstdio = 'inherit', silent = false} = {}) {
    ...
    this.log.verbose('Running %s %j in %s with options %j', id, cmd, cwd, {mode, detached});
    ...
    return Promise.try(() => {
      ...
      if (mode === 'exec' && detached === false) {
        ...
        // @NOTE: sadly we cant really do much here in terms of manipulating the process
    ...
    .then(({code, stdout, stderr}) => {
      ...
      this.log.debug('STDOUT %s', stdout);
      this.log.debug('STDERR %s', stderr);
      // Return
