 ...
 */
const spawn = (run, {stdio}, silent = false, listener, stdout = '', stderr = '') => {
  // Run the spawn
  return new Promise(resolve => {
    if (stdio === 'pipe' || stdio[1] === 'pipe') {
      run.stdout.on('data', buffer => {
        listener.emit('stdout', buffer);
...
module.exports = class Shell {
  ...
  sh(cmd, {mode = 'exec', detached = false, cwd = process.cwd(), cstdio = 'inherit', silent = false} = {}) {
    ...
    return Promise.try(() => {
      ...
      return spawn(run, stdio, silent, this.listen);
    })
