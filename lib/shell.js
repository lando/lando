...
let _shell = require('shelljs');
let _spawn = require('child_process').spawn;
let _esc = require('shell-escape');
...
module.exports = function(logger) {
   ...
   */
  let exec = function(cmd, opts) {
    // Merge in our options
    let defaults = {silent: true};
    let options = (opts) ? _.extend(defaults, opts) : defaults;
   ...
   */
  let spawn = function(cmd, opts) {
    // Promisify the spawn
    return new Promise(function(resolve, reject) {
      // Merge provided options with defaults
      let defaults = {stdio: ['pipe', 'pipe', 'pipe']};
      let options = (opts) ? _.extend(defaults, opts) : defaults;
      ...
      // Run the spawn
      let run = _spawn(entrypoint, cmd, options);
  ...
  let sh = function(cmd, opts) {
    ...
    if (opts && (opts.mode || opts.detached === true)) {
      ...
      let stdinMode = (process.lando !== 'node') ? 'ignore' : process.stdin;
      let stdErrMode = (process.lando !== 'node') ? 'ignore' : process.stderr;

      // Determine attach and collect stdio
      let collect = {stdio: [stdinMode, 'pipe', stdErrMode]};
      let attach = {stdio: 'inherit'};
      let stdio = (opts.mode === 'attach') ? attach : collect;

      // Merge in our options
      let options = (opts) ? _.extend(opts, stdio) : stdio;
   ...
   */
  let escSpaces = function(s, platform) {
    let p = platform || process.platform;
