...
exports.updatePath = function(dir) {
  ...
  const p = (process.platform === 'win32') ? 'Path' : 'PATH';
...
exports.defaults = function() {
  ...
  if (process.platform === 'win32') {
    ...
    const programFiles2 = process.env.ProgramW6432;
    const gitBin1 = path.join(appData, 'Programs', 'Git', 'usr', 'bin');
    const gitBin2 = path.join(programFiles, 'Git', 'usr', 'bin');
    const gitBin3 = path.join(programFiles2, 'Git', 'usr', 'bin');
