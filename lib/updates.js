...
module.exports = class UpdateManager {
  ...
  constructor() {
    ...
    this.prereleaseOrder = ['alpha', 'beta', 'rc', 'aft', 'rrc'];
  };
  ...
  updateAvailable(version1, version2) {
    // If we have two different prerelease tags lets compute
    if (semver.prerelease(version1) && semver.prerelease(version2)) {
      const version1Series = _.first(semver.prerelease(version1));
      const version2Series = _.first(semver.prerelease(version2));
      if (version1Series !== version2Series) {
        return _.indexOf(this.prereleaseOrder, version1Series) < _.indexOf(this.prereleaseOrder, version2Series);
      }
    }
    // Otherwise to the usual
  ...
  refresh(version) {
    ...
    .then(data => {
      ...
      const latest = _.find(_.get(data, 'data', []), r => (r.draft === false && r.prerelease === false));
