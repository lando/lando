...
module.exports = class UpdateManager {
  ...
  refresh(version) {
    ...
    return this.githubApi.repos.getReleases(landoRepoConfig)
      ...
      // @todo: is there an es6 default option thing here that can make latestrelease less cray
      .then(data => {
        // Get the latest non-draft/non-prerelease version
        const latestRelease = _.find(_.get(data, 'data', []), r => (r.draft === false && r.prerelease === false));
