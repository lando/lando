...
exports.getGlobals = app => exports.toObject(app.services, {
  ...
  labels: app.labels,
  volumes: [
    `${app.root}:/app:delegated`,
    `${app._config.userConfRoot}/scripts:/helpers`,
  ],
});
...
exports.metricsParse = app => {
  ...
  if (_.has(app, 'email')) data.email = _.get(app, 'email');
  else if (_.has(app, 'meta.email')) data.email = _.get(app, 'meta.email');
