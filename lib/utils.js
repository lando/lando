...
exports.getAppMounts = app => _(app.services)
  ...
  // Set the default
  .map(config => _.merge({}, config, {app_mount: _.get(config, 'app_mount', 'delegated')}))
 ...
 * on the appName itself
 */
exports.appMachineName = data => _.replace(_.toLower(data), / /g, '-');
...
exports.shellEscape = (command, wrap = false) => {
  ...
  if (_.isString(command)) command = parseArgsStringToArgv(command);

  // Do some crazy combintronix to help handle --option="spaced things"
  // @NOTE: this is probably going to bite me in the ass one day
  command = _(command).map(piece => {
    if (!_.includes(piece, '=')) return piece;
    else {
      const parts = piece.split('=');
      parts[1] = _.trim(parts[1], '"');
      return parts.join('=');
    }
  })
  .value();
