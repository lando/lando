 ...
 */
var getClient = function(id, url) {

  // Get URL of metrics REST API from metrics config.
  var endpoint = url || config.stats[0].url;
...
var reportInternal = function(data) {
  ...
  .each(function(endpoint) {
    getClient(id, endpoint.url).report(data)

    // Handle errors, like unable to reach given url
    .catch(function(err) {
      log.debug(err);
    });
  })
