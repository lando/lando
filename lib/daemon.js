...
// Helper for installation docs
const docsBase = 'https://docs.devwithlando.io';
const installDocs = (platform = process.platform) => {
  switch (platform) {
    case 'darwin': return `${docsBase}/installation/macos.html`;
    case 'linux': return `${docsBase}/installation/linux.html`;
    case 'win32': return `${docsBase}/installation/windows.html`;
  }
};

// Helper for docker installation fail
const noDocker = (docs = installDocs()) => ([
  '',
  '',
  'Congratulations! It should be impossible to get here!',
  '',
  'Usually this means you have disobeyed and installed Docker differently than Lando wants',
  `Please consult the documentation at ${docs}`,
  '',
  'And make sure you have installed Docker in the recommended fashion!',
  '',
]).join(os.EOL);

// Helper for docker errors
const noDockerError = (expected, detected) => ([
  noDocker(),
  `Docker is not setup quite the way Lando expects. We expected ${expected} but detected ${detected}`,
]).join(os.EOL);
...
module.exports = class LandoDaemon {
  ...
  isUp(log, cache, docker) {
    if (_.toString(shell.which(docker)).toUpperCase() !== docker.toUpperCase()) {
      log.warn('Lando thinks you might need some help with your droid!');
      const expected = docker.toUpperCase();
      const detected = _.toString(shell.which(docker)).toUpperCase();
      throw new Error(noDockerError(expected, detected));
    }
