...
/*
 * Get init type on Linux and manage service by type's command
 */
var manageServiceByInitType = function(cmd) {

  var initType = lando.shell.which('systemctl') ? 'systemctl' : 'service';

  if (initType == 'systemctl') {
...
var getServicesWrapper = function(cmd) {
  ...
  switch (process.platform) {
    ...
    case 'linux':
      return manageServiceByInitType(cmd);
    case 'win32':
