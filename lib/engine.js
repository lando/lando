...
lando.events.on('pre-bootstrap', 1, function(config) {
  ...
  config.env.KALABOX_ENGINE_ID = config.engineId;
  config.env.KALABOX_ENGINE_GID = config.engineGid;
  config.env.KALABOX_ENGINE_HOME = daemon.path2Bind4U(config.home);
  config.env.KALABOX_ENGINE_IP = dockerHost;
  var remoteIp = (process.platform === 'linux') ? dockerHost : '192.168.65.1';
  config.env.KALABOX_ENGINE_REMOTE_IP = remoteIp;
