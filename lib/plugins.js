...
// eslint-disable-next-line
const dynamicRequire = (webpack) ? __non_webpack_require__ : require;

class Plugins {
  constructor(logger) {
    this.loadedModules = [];
    // Set up the logger
    this.log = logger || new Log();
  }
  ...
  * @param {array|string} Paths to search for plugin
  ...
  */
  findPlugin(plugin, dirs) {
    ...
    const flattenMap = (elts, iterator) => _(elts)
        .chain()
        .map(iterator)
        .flatten()
        .value();
    ...
    const searchPaths = flattenMap(dirs, dir => _.map(subDirs,
        subDir => path.join(dir, subDir, plugin, 'index.js')));
    ...
    .then(plugins => {
      ...
      const pluginPath = _.last(plugins);
  ...
  load(plugin, dirs, injected) {
    ...
    // Load the plugin if we have a path
        .then(pluginPath => {
          ...
          if (!pluginPath) {
            this.log.warn('Could not find plugin %s', plugin);
          }
          ...
          // Try to load the plugin
            ...
            // If success report that and cache the path
                .then(() => {
                  ...
                  this.log.verbose('Plugin %s loaded from %s', plugin,
                      pluginPath);
                })
