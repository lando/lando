 ...
 */
const loadTasks = (dir, lando) => {
  _.forEach(fs.readdirSync(dir), file => {
    lando.tasks.add(path.basename(file, '.js'), require(path.join(dir, file))(lando));
  });
};
...
module.exports = class Lando {
  ...
  bootstrap() {
    ...
    .each(plugin => {
      ...
      if (fs.existsSync(plugin.tasks)) loadTasks(plugin.tasks, this);
      // Merge in config
      // @TODO: do we wnt to merge in other things besides config as well?
      if (_.has(plugin, 'data.config')) this.config = this.utils.merge(plugin.data.config, this.config);
