...
const bootstrapConfig = lando => {
  ...
  if (lando.config.experimental) lando.config.pluginDirs.push(path.join(__dirname, '..', 'experimental'));
  // Find the plugins
...
const bootstrapEngine = lando => {
  ...
  // Assess engine versions
  lando.engine.daemon.getVersions().then(data => {
    lando.cache.set('versions', _.merge({}, lando.versions, data), {persist: true});
...
module.exports = class Lando {
  ...
  getApp(startFrom = process.cwd(), warn = true) {
    ...
    this.log.info('App config files found at %j', landoFiles);
