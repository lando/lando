...
const stripRun = datum => {
  ...
  if (_.has(newDatum, 'opts.build')) delete newDatum.opts.build;
  ...
  if (_.has(newDatum, 'opts.last')) delete newDatum.opts.last;
  return newDatum;
};
  ...
  return docker.isRunning(utils.getId(datum)).then(isRunning => {
    ...
    if (!isRunning) {
      return exports.start(stripRun(datum), compose).then(() => {
        ...
        if (_.get(datum, 'opts.build', false) && !_.get(datum, 'opts.last', false)) {
          started = true;
        }
      });
    }
