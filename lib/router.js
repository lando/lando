...
// Helper to strip build stuff
const stripBuild = datum => {
  delete datum.opts.services;
  delete datum.opts.build;
  delete datum.opts.last;
};
...
exports.run = (data, compose, docker, started = true) => Promise.each(utils.normalizer(data), datum => {
  ...
  return docker.isRunning(utils.getId(datum)).then(isRunning => {
    ...
    if (!isRunning) {
      return exports.start(stripRun(datum), compose).then(() => {
        // if this is a build step and its not the last one make sure we set started = true
        // this prevents us from having to stop and then restart the container during builds
        started = _.get(datum, 'opts.build', false) && !_.get(datum, 'opts.last', false);
      });
    }
  })
  ...
  // Stop if we have to
  .tap(() => {
    // If this is the last step of a build we need to make sure all the containers are stopped and
    // we remove build step logix so lando doesnt get confused downstream
    if (_.get(datum, 'opts.build', false) && _.get(datum, 'opts.last', false)) stripBuild(datum);
    // Stop if we have to
