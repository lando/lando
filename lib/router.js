...
exports.run = (data, compose, docker, started = true) => Promise.each(utils.normalizer(data), datum => {
  ...
  .tap(() => {
    ...
    if (_.get(datum, 'opts.build', false) && _.get(datum, 'opts.last', false)) delete datum.opts.services;
    // Stop if we have to and remove build flags so lando doesnt get tripped up downstream
    if (!started || _.get(datum, 'opts.last', false)) {
      return exports.stop(stripRun(datum), compose, docker).then(() => {
