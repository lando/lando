...
var cacheApp = function(app) {
  if (!cache.get('appRegistry')) {
    cache.set('appRegistry', [], true);
  }
  if (!existsInCache(app, cache.get('appRegistry'))) {
    cache.set('appRegistry', cache.get('appRegistry').push(app), true);
  }
};

// Cache an appDir by appName in bad app data store.
var cacheBadApp = function(app) {
  if (!cache.get('appRegistryBad')) {
    cache.set('appRegistryBad', [], true);
  }
  if (!existsInCache(app, cache.get('appRegistryBad'))) {
    cache.set('appRegistryBad', cache.get('appRegistryBad').push(app), true);
  }
};
...
// Get app helped
var getAppCache = function(key) {
  // Init a promise.
  return Promise.resolve()
  .then(function() {
    if (!_.isEmpty(cache.get(key))) {
      // Return cached value.
...
exports.getApps = function(opts) {
  ...
  return getAppCache('appRegistry');
};
...
exports.getBadApps = function() {
  return getAppCache('appRegistryBad');
};
...
/**
 * Removes an app to the app registry.
 */
