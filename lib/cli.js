...
let chalk = require('yargonaut').chalk();
...
exports.parseToYargs = function(task, events) {
  ...
  _.forEach(command.builder, function(option, name) {
    if (option && option.interactive) {
      let question = option.interactive;
  ...
  command.handler = function(argv) {
    // Build commands
    let cmd = argv._[0];
    let answersEvent = ['task', cmd, 'answers'].join('-');
    let runEvent = ['task', cmd, 'run'].join('-');
    ...
    .then(function() {
      ...
      let questions = _.filter(inquiry, function(option) {
    ...
    .then(function(answers) {
      ...
      let truth = _.merge(argv, answers);
