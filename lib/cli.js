...
exports.initHeader = function() {
  ...
  lines.push('Now try running `lando start` to get rolling.');
  lines.push('');
  lines.push('Here are some vitals:');
  lines.push('');
...
exports.init = function(lando) {
  ...
  .then(function() {
    ...
    // Print a nice lando logo for iTerm users
    try {
      console.log();
      img(
        path.join(__dirname, '..', 'assets', 'lando.png'),
        {
          fallback: () => {},
          width: '66px'
        }
