...
const checkForUpdates = lando => {
  ...
  .then(() => {
    if (lando.updates.updateAvailable(lando.config.version, lando.cache.get('updates').version)) {
      console.error(lando.cli.makeArt('update', {paddingBottom: 0}));
      console.error(chalk.green(lando.cache.get('updates').url));
      console.error(lando.cli.makeArt('updateRC2', {paddingBottom: 0}));
    }
  });
};
...
module.exports = class Cli {
  ...
  checkPerms() {
    ...
    sudoBlock(chalk.red('Lando should never ever ever be run as root! ... like ever'));
  };
  ...
  handleError(error, handler, verbose = this.argv().verbose, lando = {}) {
    ...
    return lando.Promise.try(() => {
      if (_.isNil(lando.cache.get('report_errors'))) {
        const inquirer = require('inquirer');
        console.error(require('./art')('error'));
        const test = {
          name: 'reportErrors',
          type: 'confirm',
          default: true,
  ...
  run(tasks = [], config = {}) {
    ...
    // Secret toggle toggle
    if (yargs.argv['secret-toggle']) {
      toggleSecretToggle(this.defaultConfig().secretToggle);
      clearTaskCaches();
      process.exit(0);
    }
