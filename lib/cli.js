 ...
 */
const toggleSecretToggle = toggleFile => {
...
module.exports = class Cli {
   ...
   * Format data
   */
  formatData(input, {path = '', format = 'default', filter = {}} = {}) {
  ...
  run(tasks = [], config = {}) {
    ...
    if (yargs.argv.clear) this.clearTaskCaches();
    // Toggle secret toggle
    if (yargs.argv['secret-toggle']) this.secretToggle();
  ...
  secretToggle() {
    if (this.defaultConfig().packaged) {
      ...
      console.log(this.makeArt('secretToggle', toggleSecretToggle(this.defaultConfig().secretToggle)));
      clearTaskCaches();
      process.exit(0);
    }
  };
