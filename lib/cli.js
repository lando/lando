...
module.exports = class Cli {
  ...
  checkForUpdates(lando) {
    ...
    return lando.Promise.try(() => {
      if (lando.updates.fetch(lando.cache.get('updates'))) {
        ...
        return lando.updates.refresh(lando.config.version, lando.config.channel === 'edge')
        ...
        return lando.Promise.try(() => {
          ...
          if (lando.BOOTSTRAP_LEVELS[level] >= 3) {
            ...
            if (lando.engine.dockerInstalled === false) {
              console.log(this.makeArt('noDockerDep'));
            ...
            if (lando.engine.composeInstalled === false) {
              console.log(this.makeArt('noDockerDep', 'docker-compose'));
  ...
  run(tasks = [], config = {}) {
    ...
    if (channel) {
      ...
      const channelMessage = (channel === 'edge') ? 'edgeLiving' : 'stableLiving';
      ...
      console.log(this.makeArt(channelMessage));
    }
    if (experimental) {
