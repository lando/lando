...
const getInteractive = (options, argv) => _(options)
  ...
  .filter(option => _.isNil(argv[option.name]) || argv[option.name] === false)
  .value();
...
module.exports = class Cli {
  ...
  parseToYargs({command, describe, options = {}, run = {}, level = 'app'}) {
    const handler = argv => {
      ...
      return checkForUpdates(lando).then(() => lando.bootstrap(level).then(lando => {
         ...
         */
        return lando.events.emit(`cli-answers`, data, command)
        .then(() => lando.events.emit(`cli-${command}-answers`, data, command))
         ...
         */
        .then(answers => lando.events.emit(`cli-run`, _.merge(data.options, answers), command))
        .then(() => lando.events.emit(`cli-${command}-run`, data, command))
  ...
  run(tasks = [], config = {}) {
    ...
    yargs.usage(`Usage: ${cmd} <command> [args] [options]`)
      ...
      .recommendCommands()
      .wrap(yargs.terminalWidth() * 0.66)
      .option('clear', {describe: 'Clears the lando tasks cache', type: 'boolean'})
