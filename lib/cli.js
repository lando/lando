...
module.exports = class Cli {
  ...
  checkForUpdates(lando) {
    ...
    .then(() => {
      if (lando.updates.updateAvailable(lando.config.version, lando.cache.get('updates').version)) {
        console.error(lando.cli.makeArt('updateAvailable', lando.cache.get('updates').url));
      }
    });
  };
  ...
  defaultConfig() {
    ...
    const secretToggle = path.join(this.userConfRoot, 'secret-toggle');
    return {
      believesInFreedom: fs.existsSync(secretToggle),
      configSources: [path.join(srcRoot, 'config.yml'), path.join(this.userConfRoot, 'config.yml')],
      command: this.argv(),
      disablePlugins: fs.existsSync(secretToggle) ? [] : ['lando-alliance'],
      domain: 'lndo.site',
   ...
   * Format data
   */
  formatData(data, {path = '', format = 'default', filter = {}} = {}) {
    return formatters.formatData(data, {path, format, filter});
  };
  ...
  init(yargs, tasks, config) {
    ...
    yargs.usage(`Usage: ${cmd} <command> [args] [options]`)
      ...
      .help(false)
      .option('lando', {hidden: true, type: 'boolean'})
      .option('help', {describe: 'Shows lando or delegated command help if applicable', type: 'boolean'})
  ...
  parseToYargs({command, describe, options = {}, run = {}, level = 'app'}) {
    const handler = argv => {
      ...
      return this.checkForUpdates(lando).then(() => lando.bootstrap(level).then(lando => {
        /**
  ...
  run(tasks = [], config = {}) {
    ...
    // Toggle secret toggle
    if (yargs.argv['secret-toggle']) {
      // Deny if we are packaged
      if (this.defaultConfig().packaged) {
        console.error(this.makeArt('secretToggleDenied'));
        process.exit(1);
      }

      // Toggle
      this.secretToggle(this.defaultConfig().secretToggle);
      this.clearTaskCaches();
      console.log(this.makeArt('secretToggle', fs.existsSync(this.defaultConfig().secretToggle)));
      process.exit(0);
    }

    // Initialize
    this.init(yargs, tasks, config);
  };

  /*
   * Toggle the secret toggle
   */
  secretToggle(toggleFile) {
    if (fs.existsSync(toggleFile)) {
      fs.unlinkSync(toggleFile);
    } else {
      fs.closeSync(fs.openSync(toggleFile, 'w'));
    }
    return fs.existsSync(toggleFile);
  };
};
