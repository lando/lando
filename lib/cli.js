...
module.exports = class Cli {
   ...
   */
  defaultConfig() {
    const appRoot = this.getAppDir();
    const srcRoot = path.resolve(__dirname, '..');
    const version = require(path.join(__dirname, '..', 'package.json')).version;
    const configSources = [
      path.join(srcRoot, 'config.yml'),
      path.join(this.userConfRoot, 'config.yml'),
    ];
    const pluginDirs = [srcRoot, this.userConfRoot];

    if (appRoot) {
      configSources.push(path.join(appRoot, '.lando.yml'));
      pluginDirs.push(appRoot);
    }
  ...
  /**
   * Gets current app directory based on .lando.yml location
   *
   * Required as Lando has not yet declared the app root
   *
   * Lando will also scan parent directories if no .lando.yml is found.
   *
   * @since 3.0.0
   * @return string|false
   *
  ...
  getAppDir() {
    ...
    const files = _.map(pieces, () => {
      ...
      // @todo Keep this dynamic so that .lando.yml matches lando.config.appConfigFilename
      return path.join(dir, '.lando.yml');
    });
    ...
    // If we have a config file let's return the path
