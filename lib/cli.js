...
bootstrap() {
  ...
  return lando.bootstrap().then(lando => {
    ...
    // Kick CLI specific things off by checking for an update
   ...
   */
  parseToYargs({command, describe, options = {}}) {
    ...
    const handler = argv => {
      ...
      return this.bootstrap().then(lando => {
        /**
  ...
  run(tasks = []) {
    ...
    _.forEach(_.sortBy(tasks, 'command'), task => {
      yargs.command(this.parseToYargs(task));
    });
