...
module.exports = (options = {}) => {
  ...
  // Include app specific plugins
  if (!_.isEmpty(config.appConfigSources)) {
    const appConfig = helpers.loadFiles(config.appConfigSources);
    if (_.has(appConfig, 'plugins')) {
      config.plugins = _.concat(config.plugins, appConfig.plugins);
    }
  }
